...
// __builtin_popcount(x)    : count the number of one’s(set bits) in an integer.
// __builtin_clz(x)         : counts number of zeros before the first occurence of one(set bit). Note : clz = count leading zero’s
// __builtin_ctz(x)         : Count no of zeros from last to first occurence of one(set bit)
...
#define printBin(n , size) std::cout<<std::bitset<size>(n)
#define printDec(n) std::cout<<std::dec<<n
#define printHex(n) std::cout<<std::hex<<n
#define printOct(n) std::cout<<std::oct<<n
